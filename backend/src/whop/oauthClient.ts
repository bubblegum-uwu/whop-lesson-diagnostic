 ...
 * (src/oauth/whopOAuth.ts): the documented refresh_token grant and revoke
 * call, used to keep a stored session alive without the user re-authorizing
 * every hour.
 *
...
export function createWhopOAuthClient(clientId: string): WhopOAuthClient {
  ...
  return { refreshAccessToken, revokeRefreshToken };
}
