 ...
 */
/** Exported for reuse by http/routes/whopLessons.ts — same access-token resolution, shared verbatim rather than duplicated. */
export async function resolveAccessToken(deps: WhopCoursesRouteDeps, res: Response): Promise<string | null> {
  try {
 ...
 *
 * Phase 4K follow-up — the reverse-direction guard: also refuses to
 * connect if ANY of the course's lessons were already imported à-la-carte
 * into a DIFFERENT project (http/routes/whopLessons.ts). Without this, a
 * full-course connect could silently pull an already-à-la-carted lesson
 * (and its existing analysis) into a second project's catalog — exactly
 * the cross-project leak the à-la-carte membership table exists to
 * prevent, just approached from the other direction.
 */
export function createConnectWhopCourseHandler(deps: WhopCoursesRouteDeps) {
  return async function connectWhopCourseHandler(req: Request, res: Response): Promise<void> {
    ...
    const conflicting = await getWhopLessonImportsOwnedByOtherProject(deps.pool, lessons.map((l) => l.id), projectId);
    if (conflicting.length > 0) {
      res.status(409).json({
        error: { message: "One or more lessons in this course are already imported à la carte into a different project.", type: "lesson_already_imported_elsewhere" },
      });
      return;
    }
...
export function createListWhopCourseLessonsHandler(deps: { pool: Pool }) {
  return async function listWhopCourseLessonsHandler(req: Request, res: Response): Promise<void> {
    ...
    const statusByLesson = await getWhopLessonAnalysisStatus(deps.pool, lessonIds);
    const items = page.map((l) => {
      const entry = statusByLesson.get(l.id) ?? { status: "NOT_ANALYZED" as const, eligibleForSynthesis: false };
      return {
        id: l.id,
        title: l.title,
