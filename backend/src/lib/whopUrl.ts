/**
 * Parsing helper for Whop course lesson URLs (backend copy — kept in sync
 * with the frontend's src/lib/whopUrl.ts; duplicated because frontend and
 * backend are separate npm packages in this PoC).
 *
 * Expected shape:
...
export function parseWhopLessonUrl(rawUrl: string): ParsedWhopLessonUrl {
  ...
  if (url.hostname !== "whop.com" && url.hostname !== "www.whop.com") {
    throw new WhopUrlParseError(`Expected a whop.com URL, got hostname "${url.hostname}".`);
  }

  const segments = url.pathname.split("/").filter((s) => s.length > 0);
  const [companySlug, experienceId, appSegment, coursesSegment, courseId, lessonsSegment, lessonId] =
    segments;
  ...
  if (!experienceId.startsWith("exp_")) {
    throw new WhopUrlParseError(`Expected experience ID to start with "exp_", got "${experienceId}".`);
  }
  if (!courseId.startsWith("cors_")) {
    throw new WhopUrlParseError(`Expected course ID to start with "cors_", got "${courseId}".`);
  }
  if (!lessonId.startsWith("lesn_")) {
    throw new WhopUrlParseError(`Expected lesson ID to start with "lesn_", got "${lessonId}".`);
  }
